...
function addCards(count) {
    ...
    if (!flexContainer) {
        console.error('flex-Container не найден!');
        return;
    }
    for (let i = 0; i < count; i++) {
        ...
        card.className = 'card';
        card.innerHTML = `
                    <input type="checkbox" class="check" onclick="countSum(this)">
...
function countSum(box) {
    const sumTxt = document.getElementById('sum-text');
    const card = box.parentElement;
    let price = parseInt(((products[card.id]).price).replaceAll(' ', ''));
    if(box.checked) {
        Sum+=price;
    }else{
        Sum-=price;
    }
    let str = Sum.toString().replace(/\B(?=(\d{3})+(?!\d))/g, " ");
    sumTxt.innerHTML = `
        <p>Сумма: ${str} ₽</p>
    ` ;
}
...
window.onload = function () {
    ...
    const rndNum = Math.floor(Math.random() * products.length);
    if (rndNum == 0) {
        goshop.style.display = 'flex';
